...
export default function AdminFeedback() {
  ...
  async function load() {
    ...
    const { data, error } = await supabase
      .from('feedback')
      .select('id, name, email, message, created_at, is_read, show_as_testimonial, user_type, rating, suggestion, feature_requests, source')
        ...
        >
          <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-3">
            <div className="min-w-0">
              ...
              {r.user_type && <p className="text-xs text-gray-500 mt-1 capitalize">Role: {r.user_type}{r.rating ? ` · Rating: ${r.rating}/5` : ''}{r.source ? ` · ${r.source.replaceAll('_', ' ')}` : ''}</p>}
            </div>
          ...
          {r.suggestion && <div className="mt-3 rounded-lg bg-gray-50 p-3"><p className="text-xs font-semibold text-gray-500 uppercase">Suggestion / feature request</p><p className="text-sm text-gray-700 mt-1 whitespace-pre-wrap">{r.suggestion}</p></div>}
        </div>
      ))}
    </div>
  )
